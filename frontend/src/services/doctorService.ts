...
export interface ValidateAnalysisRequest {
  ...
  correctedHypertensionRisk?: string;
  correctedDiabetesRisk?: string;
  correctedStrokeRisk?: string;
  correctedDiabeticRetinopathyDetected?: boolean;
  correctedDiabeticRetinopathySeverity?: string;
...
const doctorService = {
  ...
  async validateAnalysis(
    analysisId: string,
    request: Omit<ValidateAnalysisRequest, 'analysisId'>
  ): Promise<any> {
    const payload: ValidateAnalysisRequest = {
      analysisId,
      ...request,
    };
    const response = await api.post<any>(`/doctors/analyses/${analysisId}/validate`, payload);
    return response.data;
  },
   ...
   * Submit AI feedback
   */
  async submitAiFeedback(request: AiFeedbackRequest): Promise<void> {
