...
public class DoctorController : ControllerBase
{
    ...
    /// Nếu truyền patientId, trả về toàn bộ phân tích của bệnh nhân đó
    /// (dùng cho trang hồ sơ bệnh nhân, có thể xem cả khi chưa được assign).
    /// </summary>
    [HttpGet("analyses")]
    ...
    public async Task<IActionResult> GetAnalyses([FromQuery] string? patientId = null)
    {
        ...
        try
        {
            // Nếu truyền patientId (trang hồ sơ bệnh nhân),
            // trả về tất cả phân tích của đúng bệnh nhân đó, không bắt buộc đã assign.
            if (!string.IsNullOrEmpty(patientId))
            {
                var analysesForPatient = await _analysisService.GetUserAnalysisResultsAsync(patientId);
                return Ok(analysesForPatient
                    .OrderByDescending(a => a.AnalysisStartedAt ?? a.AnalysisCompletedAt ?? DateTime.MinValue)
                    .ToList());
            }

            // Ngược lại, lấy danh sách phân tích của các bệnh nhân đã được assign cho bác sĩ.
            // Nếu bác sĩ chưa được assign bệnh nhân nào, fallback lấy tất cả bệnh nhân có phân tích
            // để UI vẫn hiển thị số liệu tổng quan.
            using var connection = new NpgsqlConnection(_connectionString);
            await connection.OpenAsync();
            ...
            {
                // Fallback: lấy tất cả bệnh nhân có analysis_results (không lọc theo assign)
                var allPatientsSql = @"
                    SELECT DISTINCT UserId 
                    FROM analysis_results
                    WHERE COALESCE(IsDeleted, false) = false";

                using var allPatientsCmd = new NpgsqlCommand(allPatientsSql, connection);
                using var allPatientsReader = await allPatientsCmd.ExecuteReaderAsync();
                while (await allPatientsReader.ReadAsync())
                {
                    patientIds.Add(allPatientsReader.GetString(0));
                }
    ...
    public async Task<IActionResult> GetAnalysis(string analysisId)
    {
        ...
        try
        {
            // Cho phép bác sĩ xem chi tiết phân tích của bất kỳ bệnh nhân nào
            // (vì trước đó bác sĩ đã tìm kiếm/bấm vào timeline của bệnh nhân),
            // không bắt buộc phải có record assign trước.
            using var connection = new NpgsqlConnection(_connectionString);
            await connection.OpenAsync();

            var sql = @"
                SELECT UserId FROM analysis_results
                WHERE Id = @AnalysisId 
